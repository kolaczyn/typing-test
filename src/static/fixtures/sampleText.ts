/* eslint-disable max-len */
const sampleText: string[] = [
  'Typing is the process of writing',
  'or inputting text by pressing keys',
  'on a typewriter, computer keyboard,',
  'cell phone, or calculator. It can be',
  'distinguished from other means of text',
  'input, such as handwriting and speech',
  'recognition. Text can be in the form',
  'of letters, numbers and other symbols.',
  "The world's first typist was Lillian",
  'Sholes from Wisconsin, the daughter of',
  'Christopher Sholes, who invented the',
  'first practical typewriter.',
];

export default sampleText;
